<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <input type="text" name="judul" id="judul" class="form-control form-control-md mb-3 mb-lg-0 @error('judul') is-invalid @enderror" placeholder="Ketik Judul Postingan" value="{{ old('judul') }}" required>
...
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <input type="date" name="tanggal" id="tanggal" class="form-control form-control-md mb-3 mb-lg-0 @error('tanggal') is-invalid @enderror" placeholder="Ketik Tanggal Diunggah" value="{{ old('tanggal') ?: now()->toDateString() }}" required>
  ...
  <div class="col-lg-9 fv-row">
    <select name="kategori" id="kategori" data-kt-select2="false" data-placeholder="Pilih..." data-allow-clear="true" class="form-select form-select-md @error('kategori') is-invalid @enderror" required>
      ...
      @foreach (['pengumuman','artikel','berita'] as $item)
        <option value="{{ $item }}" {{ old('kategori') == $item ? 'selected' : '' }}>{{ strtoupper($item) }}</option>
      @endforeach
    </select>
  ...
  <div class="col-lg-9 fv-row">
    <select name="status" id="status" data-kt-select2="false" data-placeholder="Pilih..." data-allow-clear="true" class="form-select form-select-md @error('status') is-invalid @enderror" required>
      @foreach (['public','private'] as $item)
        <option value="{{ $item }}" {{ old('status') == $item ? 'selected' : '' }}>{{ $item == 'public' ? 'Publik' : 'Privasi' }}</option>
      @endforeach
    </select>
...
<div class="row mb-4">
  ...
  <div class="my-1">
    <img src="/img/postingan/cover-postingan.jpg" alt="cover" class="img-fluid img-preview" style="width: 300px">
  </div>
