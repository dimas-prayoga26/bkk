  ...
  <div class="col-lg-9 fv-row">
    <select name="type" id="type" data-hide-search="true" data-kt-select2="true" data-placeholder="Pilih..." data-allow-clear="true" class="form-select form-select-md @error('type') is-invalid @enderror" required>
      ...
      @foreach (['sekolah','dudi'] as $item)
        <option value="{{ $item }}" {{ old('type', $pelamar->type ) == $item ? 'selected' : '' }}>Pelamar {{ $item == 'sekolah' ? 'Sekolah' : 'DU/DI'}}</option>
      @endforeach
    </select>
    @error('type') <span class="invalid-feedback">{{ $message }}</span> @enderror
  </div>
</div>
<div class="row mb-4 {{ ($pelamar->dudi_id == null) ? 'd-none' : '' }}" id="select-div">
  ...
  <div class="col-lg-9 fv-row">
    <select name="dudi_id" id="dudi_id" data-kt-select2="true" data-placeholder="Pilih..." data-allow-clear="true" class="form-select form-select-md @error('dudi_id') is-invalid @enderror">
      ...
      @foreach ($dudi as $item)
        <option value="{{ $item->id }}" {{ old('dudi_id', $pelamar->dudi_id ) == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
      @endforeach
    </select>
...
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <select name="jk" id="jk" data-kt-select2="true" data-placeholder="Pilih..." data-allow-clear="true" class="form-select form-select-md @error('jk') is-invalid @enderror">
      <option></option>
      @foreach (['l','p'] as $item)
...
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <select name="is_aktif" id="is_aktif" data-kt-select2="true" data-placeholder="Pilih..." data-allow-clear="true" class="form-select form-select-md @error('is_aktif') is-invalid @enderror" required>
      <option></option>
      @foreach ([1,0] as $item)
        <option value="{{ $item }}" {{ old('is_aktif', $pelamar->user->is_aktif ) == $item ? 'selected' : '' }}>{{ $item == 1 ? 'AKTIF' : 'NON-AKTIF' }}</option>
      @endforeach
    </select>
