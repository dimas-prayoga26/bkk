<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <input type="text" name="" id="pemosting" class="form-control form-control-md bg-light mb-3 mb-lg-0 @error('pemosting') is-invalid @enderror" placeholder="Ketik Pemosting" value="{{ $loker->admin->user->name }}" readonly disabled>
  </div>
</div>
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <select name="info" id="info" data-kt-select2="true" data-hide-search="true" data-placeholder="Pilih..." data-allow-clear="false" class="form-select form-select-md @error('info') is-invalid @enderror" required>
      ...
      @foreach (['internal','eksternal'] as $item)
        <option value="{{ $item }}" {{ old('info', $loker->info) == $item ? 'selected' : '' }}>{{ $item == 'internal' ? 'Internal' : 'Eksternal' }}</option>
      @endforeach
    </select>
...
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <select name="dudi_id" id="dudi_id" @can('admindudi') data-hide-search="true" @endcan data-kt-select2="true" data-placeholder="Pilih..." data-allow-clear="true" class="form-select form-select-md @error('dudi_id') is-invalid @enderror" required>
      <option></option>
      @foreach ($dudi as $item)
        <option value="{{ $item->id }}" {{ old('dudi_id', $loker->dudi_id) == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
      @endforeach
    </select>
...
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <input type="text" name="judul" id="judul" class="form-control form-control-md mb-3 mb-lg-0 @error('judul') is-invalid @enderror" placeholder="Ketik Judul Lowongan Kerja" value="{{ old('judul', $loker->judul) }}" required>
...
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <input type="text" name="posisi" id="posisi" class="form-control form-control-md mb-3 mb-lg-0 @error('posisi') is-invalid @enderror" placeholder="Ketik Posisi/Jabatan" value="{{ old('posisi', $loker->posisi) }}" required>
  ...
  <div class="col-lg-9 fv-row">
    <select name="kual_pend" id="kual_pend" data-kt-select2="true" data-placeholder="Pilih..." data-allow-clear="true" class="form-select form-select-md @error('kual_pend') is-invalid @enderror" required>
      ...
      @foreach ($kual_pend as $i => $item)
        <option value="{{ $i }}" {{ old('kual_pend', $loker->kual_pend) == $i ? 'selected' : '' }}>{{ $item }}</option>
      @endforeach
    </select>
...
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <textarea name="kual_jurusan" id="kual_jurusan" rows="3" class="form-control form-control-md mb-3 mb-lg-0 @error('kual_jurusan') is-invalid @enderror" placeholder="Ketik Kualifikasi Jurusan">{{ old('kual_jurusan', $loker->kual_jurusan) }}</textarea>
...
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <input type="date" name="tanggal_diunggah" id="tanggal_diunggah" class="form-control form-control-md mb-3 mb-lg-0 @error('tanggal_diunggah') is-invalid @enderror" placeholder="Ketik Tanggal Diunggah" value="{{ old('tanggal_diunggah', $loker->tanggal_diunggah) ?: now()->toDateString() }}" required>
...
<div class="row mb-4">
  ...
  <div class="col-lg-9 fv-row">
    <input type="date" name="tanggal_batas" id="tanggal_batas" class="form-control form-control-md mb-3 mb-lg-0 @error('tanggal_batas') is-invalid @enderror" placeholder="Ketik Tanggal Diunggah" value="{{ old('tanggal_batas', $loker->tanggal_batas) }}" required>
  ...
  <div class="col-lg-9 fv-row">
    <select name="status" id="status" data-kt-select2="false" data-placeholder="Pilih..." data-allow-clear="true" class="form-select form-select-md @error('status') is-invalid @enderror" required>
      @foreach (['buka','tutup'] as $item)
        <option value="{{ $item }}" {{ old('status', $loker->status) == $item ? 'selected' : '' }}>{{ $item == 'buka' ? 'Dibuka' : 'Ditutup' }}</option>
      @endforeach
    </select>
...
<div class="row mb-4">
  ...
  <div class="my-1">
    <img src="/img/loker/{{ $loker->cover }}" alt="cover" class="img-fluid img-preview" style="width: 300px">
  </div>
  <div class="col fv-row">
    <input type="hidden" name="old_cover" id="old_cover" value="{{ $loker->cover }}">
    <input id="cover" class="form-control form-control-sm  mb-3 mb-lg-0 @error('cover') is-invalid @enderror" type="file" name="cover_loker" onchange="previewImage()" accept="image/*">
...
<div class="row">
  <div class="col fv-row">
    ...
    <trix-editor input="isi" required>{!! old('isi', $loker->isi) !!}</trix-editor>
  </div>
